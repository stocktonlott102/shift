'use client';

import type { LessonTypeIncome } from '@/lib/types/financial';

interface LessonTypeBreakdownProps {
  data: LessonTypeIncome[];
}

function formatHours(hours: number): string {
  const h = Math.floor(hours);
  const m = Math.round((hours - h) * 60);
  if (h === 0) return `${m}m`;
  if (m === 0) return `${h}h`;
  return `${h}h ${m}m`;
}

export default function LessonTypeBreakdown({ data }: LessonTypeBreakdownProps) {
  return (
    <div className="bg-white dark:bg-neutral-800 rounded-lg shadow-sm border border-neutral-200 dark:border-neutral-700 p-4 sm:p-6">
      <h2 className="text-lg font-semibold text-neutral-900 dark:text-white mb-4">
        Income by Lesson Type
      </h2>

      {data.length === 0 ? (
        <p className="text-sm text-neutral-400 dark:text-neutral-500">
          No lesson type data for this period.
        </p>
      ) : (
        <>
          {/* Desktop Table */}
          <div className="hidden sm:block overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-neutral-200 dark:border-neutral-700">
                  <th className="text-left py-2 pr-3 font-medium text-neutral-500 dark:text-neutral-400">Type</th>
                  <th className="text-right py-2 px-2 font-medium text-neutral-500 dark:text-neutral-400">Lessons</th>
                  <th className="text-right py-2 px-2 font-medium text-neutral-500 dark:text-neutral-400">Hours</th>
                  <th className="text-right py-2 px-2 font-medium text-neutral-500 dark:text-neutral-400">Income</th>
                  <th className="text-right py-2 pl-2 font-medium text-neutral-500 dark:text-neutral-400">Avg Rate</th>
                </tr>
              </thead>
              <tbody>
                {data.map((lt) => (
                  <tr
                    key={lt.lessonTypeId || 'uncategorized'}
                    className="border-b border-neutral-100 dark:border-neutral-700/50"
                  >
                    <td className="py-2.5 pr-3">
                      <span className="flex items-center gap-2">
                        <span
                          className="w-3 h-3 rounded-full flex-shrink-0"
                          style={{ backgroundColor: lt.lessonTypeColor }}
                        />
                        <span className="font-medium text-neutral-900 dark:text-white">{lt.lessonTypeName}</span>
                      </span>
                    </td>
                    <td className="text-right py-2.5 px-2 text-neutral-700 dark:text-neutral-300">{lt.lessonCount}</td>
                    <td className="text-right py-2.5 px-2 text-neutral-700 dark:text-neutral-300">{formatHours(lt.hoursCoached)}</td>
                    <td className="text-right py-2.5 px-2 text-green-600 dark:text-green-400 font-medium">${lt.totalPaid.toFixed(2)}</td>
                    <td className="text-right py-2.5 pl-2 text-neutral-700 dark:text-neutral-300">${lt.averageRate.toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>

          {/* Mobile Cards */}
          <div className="sm:hidden space-y-3">
            {data.map((lt) => (
              <div
                key={lt.lessonTypeId || 'uncategorized'}
                className="bg-neutral-50 dark:bg-neutral-700/30 rounded-lg p-3"
              >
                <div className="flex items-center gap-2 mb-1">
                  <span
                    className="w-3 h-3 rounded-full flex-shrink-0"
                    style={{ backgroundColor: lt.lessonTypeColor }}
                  />
                  <span className="font-medium text-neutral-900 dark:text-white">{lt.lessonTypeName}</span>
                </div>
                <div className="flex items-center justify-between text-xs text-neutral-500 dark:text-neutral-400">
                  <span>{lt.lessonCount} lessons</span>
                  <span>{formatHours(lt.hoursCoached)}</span>
                  <span className="text-green-600 dark:text-green-400 font-medium">${lt.totalPaid.toFixed(2)}</span>
                </div>
                <p className="text-xs text-neutral-400 dark:text-neutral-500 mt-1">
                  Avg: ${lt.averageRate.toFixed(2)}/lesson
                </p>
              </div>
            ))}
          </div>
        </>
      )}
    </div>
  );
}
